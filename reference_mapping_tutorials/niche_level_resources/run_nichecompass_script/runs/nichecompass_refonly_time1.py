...
# %%
n_svg = 2048 # note this hyperparameter isn't used here
n_epochs = 200
n_neighbors = 4

### Dataset ###

dataset = f"BEACON_TIME_svg{n_svg}_neighbors{n_neighbors}_epochs{n_epochs}"



#adata_vis=sc.read_h5ad(ADATA_PATH + ".svg")  
...
adata_vis=adata_vis[adata_vis.obs["tech"]=="xenium"]
...
# sorted(adata.obs.sample_id.unique().to_list())
adata_vis=ad.concat([adata, adata_vis],
...
adata_vis.obs["sample"] = (
    adata_vis.obs["sample"]
    .astype(str)
    .fillna(adata_vis.obs["sample_id"].astype(str))
)
adata_vis.obs["sample"].isna().sum()
adata_vis.obs["Annotation"]=adata_vis.obs["lvl5_annotation"]
...
reference_batches = list(adata_vis.obs["sample"].unique())#.to_list()
...
for x in adata_reference.obs.columns:
    adata_reference.obs[x]=    adata_reference.obs[x].astype(str)
for x in model.adata.obs.columns:
    model.adata.obs[x]=    model.adata.obs[x].astype(str)
...
adata = sc.read_h5ad(ADATA_PATH + ".svg")
try:
